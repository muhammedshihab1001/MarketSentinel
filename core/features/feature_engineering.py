...
class FeatureEngineer:
    ...
    def _validate_price_frame(cls, df, ticker=None):
        ...
        if extreme.any():
            ...
            logger.warning(
                "Split-like move detected — repairing."
            )
    ...
    def add_core_features(cls, df):
        ...
        df["return_lag1"] = df.groupby(
            "ticker"
        )["return"].shift(1)

        df["return_lag5"] = df.groupby(
            "ticker"
        )["return"].shift(5)
    ...
    def add_cross_sectional_features(cls, df):
        ...
        if df.empty:
            ...
            logger.warning(
                "Cross-sectional width insufficient — skipping CS features"
            )
        ...
        for col in numeric_cols:
            ...
            cs_mean = clipped.groupby(
                df["date"]
            ).transform("mean")

            cs_std = clipped.groupby(
                df["date"]
            ).transform("std").clip(
                lower=cls.VAR_FLOOR
            )
    ...
    def finalize(cls, df):
        ...
        for col in missing:
            ...
            logger.warning(
                "Missing feature %s — filling with 0.0",
                col
            )
