...
# ---------------------------------------------------
# MOCK SNAPSHOT
# FIX: Matches pipeline v5.6 output shape.
#      Old shape had top_5, decision_report, agents on signals.
#      New shape: meta + executive_summary + snapshot.signals[]
#      signals[] has NO agents sub-object.
# ---------------------------------------------------
...
MOCK_SNAPSHOT = {
    ...
    # Internal fields written by pipeline for /agent/explain
    "_signal_details": {
        "NVDA": {
            "signal_agent": {
                "score": 0.82,
...
@pytest.fixture
def mock_snapshot():
    """Return a copy of the mock snapshot dict."""
    import copy
...
def mock_cache(mock_snapshot):
    """
    Mock RedisCache that returns MOCK_SNAPSHOT for the background key.
    All other gets return None.
    """
    cache = MagicMock()
...
def mock_model_loader():
    """Mock ModelLoader with a callable predict method."""
    loader = MagicMock()
...
def test_app(mock_cache, mock_model_loader):
    ...
    Bypasses real DB/Redis/model loading.
    """
    import time
    ...
    app.state.startup_time = time.time() - 120  # 2 minutes uptime
...
@pytest.fixture
def owner_token():
    """Generate a valid owner JWT for test requests."""
...
@pytest.fixture
def demo_token():
    """Generate a valid demo JWT for test requests."""
